...
Or do you want to use the entrypoints '${name}' and '${runtime}' independently on the same page with a shared runtime? In this case give them both the same value for the 'runtime' option. It must be a name not already used by an entrypoint.`);
				...
				err => {
					...
					this.codeGeneration(err => {
						...
						this._runCodeGenerationJobs(codeGenerationJobs, err => {
							...
							// ! 暂时理解为遍历所有 module ，写出带 module.buildInfo.assets 信息的资源
							// Todo: 后续要弄清楚这到底应对的是什么情况
	...
	codeGeneration(callback) {
		...
		const jobs = [];
		/**
		 * ! 这里遍历 modules 列表，为每一个module创建用于指导代码生成的job对象
		 * ? 1. module 对应的 runtime 在何时确定？猜测是在 buildChunkGraph 的时候？
		 * ? 2. runtime 的指如 Map<string, string>，key 与value 分别是什么含义？
		 * ? 3. runtime 的数量何时会大于1？
		 */
		for (const module of this.modules) {
		...
		cache.get((err, cachedResult) => {
			...
			if (!cachedResult) {
				try {
					...
					// ! result 中有一个很重要的属性： runtimeRequirements
					// ! 这里的逻辑是先生成 module 的代码，再遍历 runtime，生成runtime的代码
	...
	processRuntimeRequirements({
		...
		// ! 第一次循环：遍历 modules 收集每一个模块的 runtime 依赖
		// ! 模块的 runtime 依赖由模块对象的 codeGenerate 函数确定，这一层已经在 compilation.codeGeneration 中确定
		for (const module of modules) {
			if (chunkGraph.getNumberOfModuleChunks(module) > 0) {
				// ? 这里获取到的 runtime 是什么？在什么时候确定的？
				// ! 从数值来看，类型为 Map<string, string>
				// 获取 module 对应的 chunk 的runtime，比如:
				// 1. 配置了 `entry.xxx.runtime` 值的时候，module 对应的 runtime 就是 entry.xxx.runtime 的值
				// 2. 异步模块对应的 chunk 的 runtime 是父 chunk
				for (const runtime of chunkGraph.getModuleRuntimes(module)) {
					...
					for (const r of set) {
						// ! 这个钩子主要是 RuntimePlugin 消费，效果上看是在原有 runtimeRequirements 数组基础上增加更多 requirement
						// ! 例如对于 __webpack__require__.r 增加 __webpack_require__.*
						// ? 问题来了，为什么要做这层映射？对最终产物有什么作用？
		...
		// ! 第二次循环，遍历 chunks ，以 chunk 为单位收集该chunk 下所有 module 的 runtimeRequirements
		...
		// ! chunkGraphEntries => chunkGraph 根节点的 runtime 节点
		for (const treeEntry of chunkGraphEntries) {
			const set = new Set();
			// ? 需要验证一下，这里 getAllReferencedChunks 函数获取到的是否是所有依赖当前chunk的chunk
			...
			for (const r of set) {
				// ! 这里主要消费者同样是 RuntimePlugin
				// ! 作用是遍历 chunk 对应的所有 runtimeRequirement，并为之创建相应的 module
...
This prevents using hashes of each other and should be avoided.`);
	...
	createModuleAssets() {
		const { chunkGraph } = this;
		// ! 仅当 module.buildInfo.assets 有值的时候才会写出 module 的内容
		// ? 问题来了，这个 module.buildInfo.assets 属性是在什么地方维护的？什么情况下会有值呢？
		for (const module of this.modules) {
	...
	createChunkAssets(callback) {
		// ! module 与 runtime 都 generate 之后，执行这个动作
		// 所以这个函数应该就是最终调用 mainTemplate 的地方
		const outputOptions = this.outputOptions;

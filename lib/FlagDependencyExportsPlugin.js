					...
					(modules, callback) => {
						...
						asyncLib.each(
							...
							err => {
								...
								/**
								 * ! 这里主要就是处理每一个依赖对象，收集导出描述信息
								 * ! 导出描述信息又于具体的 dep 类型有关
								...
								const processDependency = dep => {
									...
									// ! 逐行收集 export 信息
									// ! export 信息与对应的 dep 类型强相关，只有 ${something contain "export"}Dependency
									exportsSpecsFromDependencies.set(dep, exportDesc);
								};

								/**
								 * ! 这个函数会调用 exportsInfo.getExportInfo(name) 方法，虽然是 get 但其内部实际上会创建对应对象
								 * ! 结合前面的逻辑，整个过程相当于遍历模块的 dep 列表，调用 dep.getExports 获取依赖的导出信息
								 * ! 并记录到模块的 exportsInfo 对象中
								 * ! 这就是这个插件的功能 —— 收集模块的导出信息
